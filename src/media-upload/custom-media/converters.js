...
export function viewFigureToModel() {
	...
	function converter(evt, data, conversionApi) {
		...
		const viewImage = Array.from(
			data.viewItem.getChildren()
		).find(viewChild => viewChild.is('oembed'));
...
export function srcsetAttributeConverter() {
	return dispatcher => {
		dispatcher.on('attribute:srcset:custommedia', converter);
	};
...
export function modelToViewAttributeConverter(attributeKey) {
	return dispatcher => {
		dispatcher.on(`attribute:${attributeKey}:custommedia`, converter);
	};
